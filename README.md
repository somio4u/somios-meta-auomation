...
- **One GCP service account powers everything** — Gemini via Vertex AI handles both text generation (ideas, captions, hooks, analysis) and image generation. There's no separate Anthropic/Claude key in this system, and no plain Gemini API key either (Google stopped accepting unrestricted ones in September 2026) — see `SETUP_GUIDE.md` Step 5.
