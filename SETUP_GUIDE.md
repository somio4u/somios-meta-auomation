...
- **Gemini calls fail with 403 Forbidden** (not a 404/model-not-found): this is an API key type issue, not a model name issue. Since September 2026 Google rejects unrestricted "Standard" API keys outright — only "Authorization" keys (bound to a service account) work. Go to `https://aistudio.google.com/apikey`, create a new key (keys created there now default to the Authorization type), and replace the `GEMINI_API_KEY` repository secret with it.
